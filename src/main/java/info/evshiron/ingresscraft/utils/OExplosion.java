...
import info.evshiron.ingresscraft.entities.XMPPrimed;
...
/**
 * Created by scotg_000 on 2015/6/6.
 */
public class OExplosion {
    ...
    public OExplosion(World p_i1948_1_, Entity p_i1948_2_, double p_i1948_3_, double p_i1948_5_, double p_i1948_7_, float p_i1948_9_) {
        ...
        explosion = new Explosion(p_i1948_1_,p_i1948_2_,p_i1948_3_,p_i1948_5_,p_i1948_7_,p_i1948_9_);
    }

    public OExplosion(Explosion input){
        this.explosion=input;
    }
    ...
    public void doExplosionA() {
                        ...
                        for (float f2 = 0.3F; f1 > 0.0F; f1 -= f2 * 0.75F) {
                            ...
                            if (block.getMaterial() != Material.air) {
                                float f3 = this.exploder != null ?
                                        block.getExplosionResistance(exploder, this.worldObj, j1, k1, l1, exploder.posX, exploder.posY + exploder.getEyeHeight(), exploder.posZ) : block.getExplosionResistance(this.exploder, worldObj, j1, k1, l1, explosionX, explosionY, explosionZ);
                                f1 -= (f3 + 0.3F) * f2;
                            }
        ...
        net.minecraftforge.event.ForgeEventFactory.onExplosionDetonate(this.worldObj, explosion, list, this.explosionSize);
        ...
        for (int i1 = 0; i1 < list.size(); ++i1) {
            ...
            if (d4 <= 1.0D) {
                ...
                if (d9 != 0.0D) {
                    ...
                    if(entity instanceof ResonatorEntity){
                        System.err.println("boom");
                        entity.attackEntityFrom(new EntityDamageSource("explosion.player", this.getExplosivePlacedBy()).setDifficultyScaled().setExplosion(), (float) ((int) ((d11 * d11 + d11) / 2.0D * 8.0D * (double) this.explosionSize + 1.0D)));
                    }
        ...
        if (this.isSmoking) {
            ...
            while (iterator.hasNext()) {
                ...
                if (p_77279_1_) {
                    ...
                    d4 *= d7;
                    d5 *= d7;
                    this.worldObj.spawnParticle("explode", (d0 + this.explosionX * 11.0D) / 2.0D, (d1 + this.explosionY * 11.0D) / 2.0D, (d2 + this.explosionZ * 11.0D) / 2.0D, d3, d4, d5);
    ...
    public EntityLivingBase getExplosivePlacedBy() {
        return this.exploder == null ? null : (this.exploder instanceof XMPPrimed ? ((XMPPrimed) this.exploder).getTntPlacedBy() : (this.exploder instanceof EntityLivingBase ? (EntityLivingBase) this.exploder : null));
    }
}
